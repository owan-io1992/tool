...
## Branch Prefixes
Branch names must start with a prefix corresponding to one of the following Linear labels: `feat`, `fix`, or `doc`. These labels are mutually exclusive.
If an issue does not have one of the required labels (`feat`, `fix`, or `doc`), the user should be prompted to add one before creating the branch.
